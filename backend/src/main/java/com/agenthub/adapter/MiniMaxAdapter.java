...
public class MiniMaxAdapter implements AgentAdapter {
    ...
    private static final Logger log = LoggerFactory.getLogger(MiniMaxAdapter.class);
    private static final Pattern CODE_BLOCK_PATTERN = Pattern.compile("```(\\w+)?\\n?([\\s\\S]*?)```");

    @Value("${minimax.api.key:}")
    private String apiKey;

    @Value("${minimax.api.base:https://api.minimax.chat/v1}")
    private String apiBase;

    @Value("${minimax.model:minimax-01}")
    ...
    public AgentResponse generate(AgentRequest request) {
        if (!isAvailable()) {
            return createFallbackResponse("MiniMax API key not configured");
        }
        ...
        try {
            ...
            log.debug("Calling MiniMax API with model: {}", modelName);
            ...
            return createFallbackResponse("MiniMax API error: " + e.getMessage());
        }
    }
    ...
    public Flux<String> generateStream(AgentRequest request) {
        if (!isAvailable()) {
            return Flux.just("MiniMax API key not configured");
        }
    ...
    public ProviderMeta getProviderMeta() {
        ...
        meta.setProviderCode("MINIMAX");
        meta.setProviderName("MiniMax");
    ...
    private AgentResponse parseResponse(String response) {
        ...
        try {
            ...
            log.error("Failed to parse MiniMax response", e);
    ...
    /**
     * Convert ToolDefinition list to map format for API request.
     */
    ...
    private AgentResponse createFallbackResponse(String reason) {
        ...
        response.setContent(reason + ". Please configure MINIMAX_API_KEY.");
