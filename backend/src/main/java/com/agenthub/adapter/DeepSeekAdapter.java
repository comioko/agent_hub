...
@Component
public class DeepSeekAdapter implements AgentAdapter {
    ...
    @Value("${deepseek.api.base:https://api.deepseek.com/v1}")
    ...
    public AgentResponse generate(AgentRequest request) {
        if (!isAvailable()) {
            return createFallbackResponse("DeepSeek API key not configured");
        }
        ...
        try {
            ...
            log.debug("Calling DeepSeek API with model: {}", modelName);
            ...
            return createFallbackResponse("DeepSeek API error: " + e.getMessage());
        }
    }
    ...
    public Flux<String> generateStream(AgentRequest request) {
        if (!isAvailable()) {
            return Flux.just("DeepSeek API key not configured");
        }
    ...
    public ProviderMeta getProviderMeta() {
        ...
        meta.setProviderCode("DEEPSEEK");
        meta.setProviderName("DeepSeek");
    ...
    private AgentResponse createFallbackResponse(String reason) {
        ...
        response.setContent(reason + ". Please configure DEEPSEEK_API_KEY.");
